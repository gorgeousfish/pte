...
if _rc {
    di as error "pte_replication.do could not find `root'/data/复现数据.dta"
    di as error "Run this example from the repository root or pass the repo root as the first do-file argument."
    exit 601
}
...
* Generate panel ID and industry variable from raw data
...
forvalues j = 1/`n_ind' {
    ...
    * Reload data for each industry (clean state)
    quietly use "`root'/data/复现数据.dta", clear
    quietly egen firm = group(Scode)
    quietly gen ind = indid
gen D = treat
