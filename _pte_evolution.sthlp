...
{p2colset 5 24 26 2}{...}
{p2col:{cmd:_pte_evolution} {hline 2}}Estimate the EPIC-002 productivity evolution law{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab:Required}
{synopt:{opt treatment(name)}}binary treatment-state variable used as {it:D_t}{p_end}

{syntab:Optional}
{synopt:{opt omegapoly(#)}}polynomial order of the evolution law; 1, 2, 3, or 4; default {cmd:3}{p_end}
{synopt:{opt pfunc(string)}}production-function branch metadata to forward; {cmd:cd} or {cmd:translog}{p_end}
{synopt:{opt touse(name)}}active-sample indicator; the name must match an existing numeric variable exactly, and current and lag rows must both satisfy {cmd:touse!=0}{p_end}
...
{pstd}
{cmd:_pte_evolution} is the EPIC-002 step that estimates the realized-productivity
evolution regression used by the PTE package after {cmd:omega} has been recovered.
...
{pstd}
Only non-transition observations are admissible in the current period. The lagged
state must also come from the active sample. In package terms, {cmd:_pte_evolution}
...
{phang2}{cmd:e(N_lag_untreated)} counts admissible rows with {cmd:D_{t-1}=0}.{p_end}
{phang2}{cmd:e(N_lag_treated)} counts admissible rows with {cmd:D_{t-1}=1}.{p_end}
{phang2}{cmd:e(lag_treated_supported)} is 1 only when the treated evolution law is identified in the live OLS fit. It is 0 both when there is no admissible treated lag support at all and when treated-side regressors are omitted by the live OLS fit.{p_end}
...
the command first reuses {cmd:e(prodfunc)} and then the legacy alias {cmd:e(pfunc)}
...
EPIC-002 bridge ({cmd:_pte_prodfunc}, {cmd:_pte_omega_recovery}, {cmd:_pte_evolution},
...
{synopt:{cmd:e(rho2)}}quadratic untreated coefficient if {cmd:omegapoly>=2}{p_end}
...
{synopt:{cmd:e(gamma1)}}linear treated-minus-untreated interaction coefficient; posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(gamma2)}}quadratic treated-minus-untreated interaction coefficient if {cmd:omegapoly>=2}; posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(gamma3)}}cubic treated-minus-untreated interaction coefficient if {cmd:omegapoly>=3}; posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(gamma4)}}quartic treated-minus-untreated interaction coefficient if {cmd:omegapoly>=4}; posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(delta)}}direct treated intercept shift; posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(omegapoly)}}polynomial order used in the regression{p_end}
{synopt:{cmd:e(N)}}admissible evolution-regression observations (= {cmd:e(N_evo)}){p_end}
{synopt:{cmd:e(N_evo)}}admissible evolution-regression observations{p_end}
{synopt:{cmd:e(N_lag_untreated)}}non-transition rows with untreated lag support{p_end}
{synopt:{cmd:e(N_lag_treated)}}non-transition rows with treated lag support before any OLS omission check{p_end}
{synopt:{cmd:e(lag_treated_supported)}}1 if the treated evolution law is identified in the live OLS fit; 0 on the h_bar_0-only fallback path, whether because treated lag support is absent or because treated-side regressors are omitted{p_end}
{synopt:{cmd:e(r2)}}R-squared from the evolution regression{p_end}
{synopt:{cmd:e(rmse)}}RMSE from the evolution regression{p_end}
{synopt:{cmd:e(xtdelta)}}stored panel spacing from the live estimation axis when available{p_end}

{synoptset 22 tabbed}{...}
{p2col 5 22 26 2: Matrices}{p_end}
{synopt:{cmd:e(b)}}regression coefficient row vector from the live OLS fit; coefficient labels use the DO-compatible base names {cmd:L.omega}, {cmd:L.omega2}, {cmd:L.omega3}, {cmd:L.omega_tp}, ..., rather than the internal working-variable names. {cmd:predict, xb} is supported through a custom prediction routine that rebuilds the fitted law directly from the live posted coefficient vector, so the command does not need to publish user-visible helper variables such as {cmd:omega_tp}. When Stata omits a treated-side regressor, the omitted-term prefix is preserved in the live labels, so entries may appear as {cmd:oL.omega_tp}, {cmd:oL.omega2_tp}, or {cmd:oL.<treatment>} using the exact variable name supplied in {opt treatment()}; the custom predictor skips those omitted entries but still uses any surviving treated-side coefficients so the posted consumer path remains equal to the live OLS fit that created {cmd:_pte_omega_hat}.{p_end}
{synopt:{cmd:e(V)}}regression covariance matrix from the live OLS fit with the same live dimnames as {cmd:e(b)}, including any omitted-term {cmd:o.} prefixes that Stata preserves{p_end}
{synopt:{cmd:e(rho_0)}}row vector of untreated evolution coefficients [{cmd:rho0 rho1 ...}]{p_end}
{synopt:{cmd:e(rho_1)}}row vector of treated evolution coefficients [{cmd:rho0+delta rho1+gamma1 ...}]; posted only when effective treated-lag support exists{p_end}
...
{synoptset 18 tabbed}{...}
{synopt:{cmd:_pte_active_sample}}byte; persisted EPIC-002 active-sample / bridge-state indicator written after a successful live evolution fit. Downstream reruns such as {cmd:_pte_eps0_sample}, {cmd:_pte_winsorize}, and bridge re-entry into {cmd:_pte_evolution} reuse this variable when {opt touse()} is omitted, so failed reruns do not silently overwrite the last successful active-sample boundary.{p_end}
{synopt:{cmd:_pte_omega_hat}}double; fitted values from the evolution regression on {cmd:e(sample)}. The command clears any stale copy at entry and recreates this variable only after a successful live regression, so failed reruns do not leave an obsolete hat behind for downstream eps0 recovery.{p_end}
