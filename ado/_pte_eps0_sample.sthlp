...
{p2colset 5 26 28 2}{...}
{p2col:{cmd:_pte_eps0_sample} {hline 2}}Build the untreated innovation support
for EPIC-002{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab:Required}
{synopt:{opt treatment(name)}}binary treatment-state variable; must match an
existing numeric variable exactly and equal the current EPIC-002 treatment
state{p_end}

{syntab:Optional}
{synopt:{opt eps0window(#)}}nonnegative common pre-treatment window; {cmd:0}
keeps the full identified untreated support{p_end}
{synopt:{opt touse(name)}}exact numeric active-sample indicator; current and lag
rows must both satisfy {cmd:touse!=0}{p_end}
...
{phang2}1. Computes each firm's observed treatment-entry year
{cmd:_pte_treat_year} from the full observed treatment path; left-censored
treated firms keep this metadata missing because no observed {cmd:0->1} entry is
available in sample.{p_end}
{phang2}2. Marks the untreated innovation support {cmd:_pte_eps0_ind==1}.{p_end}
{phang2}3. Rebuilds the untreated fitted law
{cmd:_pte_omega_hat = h_bar_0(L.omega)} from the live {cmd:e(rho_0)}
coefficients.{p_end}
{phang2}4. Stores the untreated innovation residuals
{cmd:_pte_eps0 = omega - _pte_omega_hat}.{p_end}
...
{cmd:anchor_year - # * delta() <= year < anchor_year}, where {cmd:anchor_year}
is the
observed entry year of the earliest treated cohort that remains active in the
current estimation sample and still retains admissible untreated evolution
...
existing numeric variable exactly. The current row and the lag row that supplies
{cmd:L.omega} /
...
{synopt:{cmd:e(N_eps0_treated)}}untreated innovation observations from
eventually treated firms{p_end}
{synopt:{cmd:e(N_eps0_control)}}untreated innovation observations from
never-treated firms{p_end}
...
{synopt:{cmd:e(sigma_eps)}}raw standard deviation of the current untreated
innovation support{p_end}
{synopt:{cmd:e(sigma_eps_trim)}}1%-99% trimmed standard deviation of the current
untreated innovation support; if the live EPIC-002 bridge already has
{cmd:e(trimeps)=0}, this aliases {cmd:e(sigma_eps)} on the rebuilt
support{p_end}
{synopt:{cmd:e(N_eps0_trim)}}observations retained by the active trim law on the
current support; equals {cmd:e(N_eps0)} when the live bridge has
{cmd:e(trimeps)=0}{p_end}
{synopt:{cmd:e(eps0_p1)}}1st percentile cutoff of the current untreated
innovation support when trimming is active; missing when the live bridge has
{cmd:e(trimeps)=0}{p_end}
{synopt:{cmd:e(eps0_p99)}}99th percentile cutoff of the current untreated
innovation support when trimming is active; missing when the live bridge has
{cmd:e(trimeps)=0}{p_end}
{synopt:{cmd:e(trimeps)}}trim indicator forwarded from the live EPIC-002 bridge
when available; otherwise defaults to the canonical trimmed-Gaussian path
({cmd:1}){p_end}
{synopt:{cmd:e(omegapoly)}}evolution-polynomial order forwarded from the live
EPIC-002 state{p_end}
{synopt:{cmd:e(xtdelta)}}stored panel spacing forwarded from the live EPIC-002
state when available{p_end}
{synopt:{cmd:e(rho0)}}untreated evolution intercept{p_end}
{synopt:{cmd:e(rho1)}}untreated linear coefficient{p_end}
{synopt:{cmd:e(rho2)}}untreated quadratic coefficient if
...
{synopt:{cmd:e(gamma1)}}treated-minus-untreated linear interaction; posted only
when treated-lag support exists{p_end}
{synopt:{cmd:e(gamma2)}}treated-minus-untreated quadratic interaction if
{cmd:omegapoly>=2}; posted only when treated-lag support exists{p_end}
{synopt:{cmd:e(gamma3)}}treated-minus-untreated cubic interaction if
{cmd:omegapoly>=3}; posted only when treated-lag support exists{p_end}
{synopt:{cmd:e(gamma4)}}treated-minus-untreated quartic interaction if
{cmd:omegapoly>=4}; posted only when treated-lag support exists{p_end}
{synopt:{cmd:e(delta)}}treated intercept shift from the live evolution state;
posted only when treated-lag support exists{p_end}
{synopt:{cmd:e(N_evo)}}admissible evolution-regression observations{p_end}
{synopt:{cmd:e(N_lag_untreated)}}rows with untreated lag support, when available
upstream{p_end}
{synopt:{cmd:e(N_lag_treated)}}rows with treated lag support, when available
upstream{p_end}
{synopt:{cmd:e(lag_treated_supported)}}1 if the treated evolution law is
identified in the live OLS fit; 0 otherwise, when available upstream{p_end}
{synopt:{cmd:e(r2)}}compatibility alias for the forwarded evolution
R-squared{p_end}
{synopt:{cmd:e(rmse)}}compatibility alias for the forwarded evolution
RMSE{p_end}
{synopt:{cmd:e(r2_evo)}}standardized EPIC-002 alias for the forwarded evolution
R-squared{p_end}
{synopt:{cmd:e(rmse_evo)}}standardized EPIC-002 alias for the forwarded
evolution RMSE{p_end}

{synoptset 22 tabbed}{...}
{p2col 5 22 26 2: Matrices}{p_end}
{synopt:{cmd:e(rho_0)}}untreated evolution coefficient row vector{p_end}
{synopt:{cmd:e(rho_1)}}treated evolution coefficient row vector; posted only
when treated-lag support exists{p_end}
...
{synopt:{cmd:e(id)}}stored panel identifier alias forwarded from the live
EPIC-002 state{p_end}
{synopt:{cmd:e(time)}}stored time variable alias forwarded from the live
EPIC-002 state{p_end}
{synopt:{cmd:e(idvar)}}stored estimation panel identifier used when
reconstructing eps0 support{p_end}
{synopt:{cmd:e(timevar)}}stored estimation time variable used when
reconstructing eps0 support{p_end}
...
{synoptset 20 tabbed}{...}
{synopt:{cmd:_pte_treat_year}}observed treatment-entry year; missing for
never-treated and left-censored treated firms{p_end}
{synopt:{cmd:_pte_eps0_ind}}indicator for the identified untreated innovation
support{p_end}
{synopt:{cmd:_pte_omega_hat}}untreated fitted evolution law on the selected
support{p_end}
{synopt:{cmd:_pte_eps0}}untreated innovation residuals on the selected
support{p_end}
...
{pstd}Default identified untreated support after running EPIC-002
evolution:{p_end}
...
{pstd}Bridge from the treatdependent evolution worker before rebuilding
eps0:{p_end}
