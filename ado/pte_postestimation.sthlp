...
matches are not accepted on this fallback path. {cmd:predict} also reuses {cmd:e(xtdelta)} when
...
fallback path. If {cmd:eps0window()>0} and the current data contain neither {cmd:_pte_eps0} nor {cmd:_pte_eps0_ind}, {cmd:predict, residuals} cannot rebuild eps0 safely because the window-restricted untreated support is no longer observable. Re-run pte before predict, residuals, or keep _pte_eps0/_pte_eps0_ind in the current data.
...
{p2col:{cmd:_pte_tt}}firm-specific treatment effect TT_it on the canonical trimmed-Gaussian paper track{p_end}
{p2col:{cmd:_pte_tt_raw}}firm-specific treatment effect TT_it on the raw (untrimmed) track{p_end}
{p2colreset}{...}
...
{synopt:{cmd:e(xtdelta)}}stored panel time spacing used when residuals restores {cmd:xtset}, when available{p_end}
{synopt:{cmd:e(att_0)}}compatibility alias for ATT at event time 0, when stored{p_end}
{synopt:{cmd:e(att_1)}}compatibility alias for ATT at event time 1, when stored{p_end}
{synopt:{cmd:e(att_}{it:s}{cmd:)}}compatibility alias for ATT at event time {it:s}, when stored{p_end}
...
{synopt:{cmd:e(attperiods)}}1x(L+1) row vector of exact integer ATT event-time periods (nt list){p_end}
{synopt:{cmd:e(att)}}1x(L+2) row vector of ATT estimates; last column is overall ATT{p_end}
...
{synopt:{cmd:e(idvar)}}exact stored panel identifier used by {cmd:predict, residuals}{p_end}
{synopt:{cmd:e(timevar)}}exact stored time variable used by {cmd:predict, residuals}{p_end}
{synopt:{cmd:e(id)}}legacy stored panel identifier used as fallback by {cmd:predict, residuals}{p_end}
...
{synopt:{cmd:e(groups)}}stored grouped-estimation labels used by grouped {cmd:predict, residuals} and {cmd:predict, att}, including grouped bootstrap public reposts that retain grouped mapping payloads; grouped {cmd:predict, parameters} keeps those labels in {cmd:e(groups)} instead of reprinting the raw token list{p_end}
{synoptline}
...
{pstd}
Bug reports and suggestions: {browse "https://github.com/pte-stata/pte":github.com/pte-stata/pte}{p_end}
