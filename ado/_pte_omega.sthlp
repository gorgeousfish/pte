...
{p2colset 5 20 22 2}{...}
{p2col:{cmd:_pte_omega} {hline 2}}EPIC-002: Productivity Recovery and Evolution{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab:Required}
{synopt:{opt treatment(name)}}treatment indicator variable; must be the exact live treatment-state variable name (no abbreviation fallback){p_end}

{syntab:Evolution}
{synopt:{opt omegapoly(#)}}polynomial order for evolution (1-4, default 3){p_end}
{synopt:{opt eps0window(#)}}if positive, restrict eps0 to the # panel periods before the observed treatment-entry year among treated cohorts that remain active in the current estimation sample and still retain admissible untreated evolution support, scaled by the current {cmd:xtset} {cmd:delta()}; left-censored treated firms have no identifiable observed entry year and cannot anchor the window; default uses the full identified untreated support ({cmd:0}){p_end}
...
{synopt:{opt prodfunc(string)}}production function type ("cd" or "translog"); if omitted, standalone recovery infers {cmd:translog} from higher-order coefficients, otherwise the live EPIC-001 metadata {cmd:e(prodfunc)} with legacy fallback {cmd:e(pfunc)} are used before defaulting to {cmd:cd}{p_end}
{synopt:{opt touse(name)}}active-sample indicator; the name must match an existing numeric variable exactly, recovery uses current rows with {cmd:touse!=0}, and lagged evolution regressors must also come from rows with {cmd:touse!=0}{p_end}
...
{pstd}
{cmd:_pte_omega} is the orchestrator module for EPIC-002 (Productivity Recovery and
Evolution) in the {cmd:pte} package. It coordinates the complete workflow:

{phang2}{bf:Step 1 (US-E2-001):} Productivity recovery: omega = phi - f(k,l;beta){p_end}
{phang2}{bf:Step 2 (US-E2-002):} Evolution regression with polynomial and interactions{p_end}
{phang2}{bf:Step 3 (US-E2-003):} eps0 sample selection (control group pre-treatment){p_end}
{phang2}{bf:Step 4 (US-E2-004):} Winsorize and normal approximation for eps0{p_end}

{pstd}
This module implements the theoretical framework from Chen, Liao & Schurter (2026)
...
{phang2}{cmd:e(rho_0)}: Control group evolution function h_bar_0 = [rho_0, rho_1, ..., rho_p]{p_end}
{phang2}{cmd:e(rho_1)}: Treated group evolution function h_bar_1 = [rho_0+delta, rho_1+gamma_1, ...]{p_end}
...
step continues with the pooled regression. In that case {cmd:h_bar_0 remains estimable}
...
{pstd}
{bf:Prerequisites:} This module requires EPIC-001 to have been executed, providing:

{phang2}- Variable {cmd:phi} (control-adjusted first-stage productivity proxy; first-stage controls already removed){p_end}
{phang2}- Variable {cmd:_pte_mid} (package transition-period indicator; {cmd:mid} is accepted only as a legacy fallback compatibility alias){p_end}
...
within {it:#} panel periods before the observed treatment-entry year among treated cohorts
...
pre-treatment support among admissible cohorts, matching the general Proposition 4.3 identification
...
treatment-entry year from the full observed panel path. If {opt eps0window(#)} exceeds the
...
is computed after the active-sample and lag-source restrictions, so {opt touse()}
...
{phang}
{opt notrimeps} disables Winsorize trimming of the eps0 distribution. When specified,
...
pool (except for the documented official translog Gaussian exceptions). This is {bf:not recommended}
per Section 6.3.3 of the paper.

{dlgtab:Production Function}

{phang}
{opt beta_l(#)}, {opt beta_k(#)}, etc. specify the production function coefficients
from EPIC-001. These are used to recover productivity: omega = phi - f(k,l;beta).
...
{synopt:{cmd:e(gamma1)}}first-order interaction coefficient; posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(gamma2)}}second-order interaction coefficient (if omegapoly >= 2); posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(gamma3)}}third-order interaction coefficient (if omegapoly >= 3); posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(gamma4)}}fourth-order interaction coefficient (if omegapoly >= 4); posted only when effective treated-lag support exists{p_end}
{synopt:{cmd:e(delta)}}treatment direct effect; posted only when effective treated-lag support exists{p_end}

{synoptset 20 tabbed}{...}
{p2col 5 20 24 2: Scalars - Sample Sizes}{p_end}
{synopt:{cmd:e(N)}}observations in the active omega-recovery sample (= {cmd:e(N_omega)}){p_end}
{synopt:{cmd:e(N_omega)}}observations with valid omega{p_end}
{synopt:{cmd:e(N_evo)}}observations in evolution regression{p_end}
{synopt:{cmd:e(N_lag_untreated)}}non-transition evolution rows with valid untreated lag support ({cmd:D_{t-1}=0}){p_end}
{synopt:{cmd:e(N_lag_treated)}}non-transition evolution rows with valid treated lag support before the live OLS omission check ({cmd:D_{t-1}=1}){p_end}
...
{synopt:{cmd:e(sigma_eps_trim)}}trimmed standard deviation used by the canonical Gaussian ATT track{p_end}
...
{synopt:{cmd:e(eps0window)}}eps0 innovation-window width in panel periods ({cmd:0} = full identified untreated support among admissible cohorts){p_end}
{synopt:{cmd:e(lag_treated_supported)}}1 if effective lag-treated support exists after the live OLS omission check, 0 if the pooled regression continued with treated-lag terms unsupported{p_end}
{synopt:{cmd:e(trimeps)}}1 if Winsorize enabled, 0 otherwise{p_end}
{synopt:{cmd:e(xtdelta)}}stored panel spacing from the live EPIC-002 estimation axis when available{p_end}

{synoptset 20 tabbed}{...}
{p2col 5 20 24 2: Matrices}{p_end}
{synopt:{cmd:e(rho_0)}}control group evolution parameters [rho_0, rho_1, ..., rho_p]{p_end}
{synopt:{cmd:e(rho_1)}}treated group evolution parameters [rho_0+delta, rho_1+gamma_1, ...]; posted only when effective treated-lag support exists{p_end}

{synoptset 20 tabbed}{...}
{p2col 5 20 24 2: Macros}{p_end}
{synopt:{cmd:e(treatment)}}treatment variable name{p_end}
{synopt:{cmd:e(treatsig)}}current treatment-law signature for downstream ATT / graph certification on the live panel axis{p_end}
{synopt:{cmd:e(pfunc)}}legacy alias for the production function type (same value as {cmd:e(prodfunc)}){p_end}
{synopt:{cmd:e(prodfunc)}}production function type{p_end}
{synopt:{cmd:e(id)}}stored panel identifier alias for downstream bridge workers{p_end}
{synopt:{cmd:e(time)}}stored time variable alias for downstream bridge workers{p_end}
{synopt:{cmd:e(idvar)}}stored estimation panel identifier for downstream bridge workers{p_end}
{synopt:{cmd:e(timevar)}}stored estimation time variable for downstream bridge workers{p_end}
{synopt:{cmd:e(eps0_dist)}}"normal"; EPIC-002 Gaussian approximation label for untreated innovations, while downstream ATT still distinguishes the empirical raw track and official translog Gaussian exceptions{p_end}
...
{phang}
Replication code: {cmd:DOs/prodest_3rd_poly.do}, {cmd:DOs/tt_estimation_program.do}
